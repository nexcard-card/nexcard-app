...
class _AppWebViewState extends State<AppWebView> {
  ...
  Future<void> _loadApp() async {
    try {
      ...
      _controller.loadHtmlString(htmlContent, baseUrl: 'file:///android_asset/flutter_assets/assets/');
    } catch (e) {
      setState(() {
        _hasError = true;
        _isLoading = false;
            ...
            if (_hasError)
              Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    const Icon(Icons.error_outline, color: Color(0xFF0099aa), size: 60),
                    const SizedBox(height: 16),
                    const Text('ط®ط·ط£ ظپظٹ ط§ظ„طھط­ظ…ظٹظ„', style: TextStyle(color: Colors.white, fontSize: 18)),
                    const SizedBox(height: 16),
                    ElevatedButton(
                      onPressed: _loadApp,
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0099aa)),
